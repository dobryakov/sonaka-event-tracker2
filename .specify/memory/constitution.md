...
**Клиентские контейнеры не должны постоянно висеть** в составе стека как долгоживущие сервисы. Ожидаемый цикл: **однократный запуск** → **срабатывание** сценария (отправка события, интеграционный тест и т.п.) → **выход** процесса с кодом. Для клиентов используются разовые вызовы (`docker compose run`, целевая команда через `exec` и аналоги), а не удержание контейнера в idle; если когда-либо потребуется иной режим, это фиксируется явно в спецификации.
